...
- **K8s MCP Server Onboarding Skill (`.agents/skills/k8s-mcp-server-onboarding/`)**: Ogni volta che viene richiesta l'integrazione, containerizzazione o il deployment di un nuovo server Model Context Protocol (MCP) nel cluster Kubernetes (`mcp-system`), l'agente DEVE obbligatoriamente attivare e seguire lo skill dedicato `k8s-mcp-server-onboarding`. Lo skill governa l'intero ciclo: scelta del pattern (`mcp-registry-package` vs `mcp-upstream-tracking`), Dockerfile unprivileged (UID 1000, read-only rootfs), divieto di `:latest`, segreti SOPS (`mcp-secret-projection`), dichiarazione GitOps in `mcp-gateway-values.yaml`, DNS Unbound su OPNsense e configurazione centralizzata in `~/.gemini/antigravity/mcp_config.json`.
