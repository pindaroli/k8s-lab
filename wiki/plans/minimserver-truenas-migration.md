...
- Config persistente: NFS `10.10.20.50:/mnt/stripe/k8s-arr/servarr-minimserver` montato su `/opt/minimserver/data`. Libreria in sola lettura (`contentDir = /media/classical`).
- Stack di destinazione: `servarr/truenas-docker/docker-compose.yaml` su `10.10.20.50`. Il compose di failover in `servarr/compose/` è superato da [[servarr-truenas-permanent-migration]] e non va toccato.
...
Il bridge Docker non inoltra SSDP. Il servizio Compose usa `network_mode: host`, come il precedente `hostNetwork`. Gli annunci UPnP escono da `10.10.20.50` (VLAN 20), lo stesso segmento del precedente `hostNetwork` su `10.10.20.142`.
...
2. **Compose.** In `servarr/truenas-docker/docker-compose.yaml`:
   ...
   - Avvio solo dopo la copia. `9790` deve rispondere su `10.10.20.50` con il pod Kubernetes ancora a zero.
3. **Traefik, stesso schema di Jellyfin.** Service ed Endpoints `minimserver-external-svc` verso `10.10.20.50:9790`. L'IngressRoute non punta più a `servarr-minimserver`. I nomi host e l'OAuth sul pubblico non cambiano. Gli A record Unbound e gli alias in `rete.json` restano sul VIP `10.10.20.56`.
4. **Homepage.** `siteMonitor` nelle homepage Kubernetes: `http://minimserver-external-svc.arr.svc.cluster.local:9790`. Gli `href` restano `https://minimserver.pindaroli.org` e `https://minimserver-internal.pindaroli.org`. Apply e rollout di `homepage` e `homepage-local`. Voce sulla homepage TrueNAS verso `http://10.10.20.50:9790`, poi restart del container `homepage`.
...
flowchart LR
  ...
  traefik --> ep[Endpoints 10.10.20.50:9790]
  ep --> docker[minimserver host network]
  docker --> media["/mnt/oliraid/arrdata/media"]
  renderer[Renderer DLNA] -->|SSDP da VLAN 20| docker
```
...
- [x] Homepage Kubernetes applicate e rollout eseguito. Tile TrueNAS su `http://10.10.20.50:9790`.
...
- [ ] Un renderer in LAN deve ancora confermare di vedere il server sulla VLAN 20.
