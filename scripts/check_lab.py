...
def run_ssh_json(host, cmd_str):
    # Esegue un comando ssh e ritorna un dizionario JSON (timeout breve per non bloccarsi)
    full_cmd = ["ssh", "-o", "StrictHostKeyChecking=no", "-o", "ConnectTimeout=3", "-o", "BatchMode=yes", f"{SSH_USER}@{host}", cmd_str]
    stdout = run_cmd(full_cmd)
    if stdout:
        try:
