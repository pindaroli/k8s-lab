#!/bin/bash
# Script per identificare lo snapshot di oliraid/arrdata/classical con il massimo numero di file in library
...
if [ "$max_count" -ge 0 ]; then
    echo "RISULTATO:"
    echo "Lo snapshot con il massimo numero di file reali (esclusi symlink) in library è:"
    echo "--> $max_snapshot"
